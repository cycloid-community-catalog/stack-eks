...
resource "aws_iam_policy" "eks-node-cluster-autoscaler" {
  name        = "${var.project}-${var.env}-eks-node-${var.node_group_name}-cluster-autoscaler"
  path        = "/"
...
data "aws_iam_policy_document" "cloudformation-signal" {
  statement {
    ...
    resources = [
      "arn:aws:cloudformation:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:stack/${var.project}-${var.env}-eks-node-${var.node_group_name}/*",
    ]
  }
}

resource "aws_iam_policy" "cloudformation-signal" {
  name        = "${var.project}-${var.env}-eks-node-${var.node_group_name}-cloudformation-signal"
  path        = "/"
